...
###  Project: Predicting Future Prices of Cocoa Beans                      ###
###                                                                        ###
###  Created by: Clifford Cele                                             ###
###........................................................................###
...
libraries = c('data.table', 'readxl', 'xts', 'forecast', 'CADFtest', 'tempdisagg', 
              'tidyverse', 'fGarch', 'ggpubr', 'vars', 'ggthemes')
...
# Cocoa Future Prices
Daily.Prices_NEW <- read.csv("../Data/Daily_Prices_NEW.csv")
...
# Extract London GBP/tonne Futures Prices
GBP.tonne.futures <- data.frame(Daily.Prices_NEW$Date,
                                Daily.Prices_NEW$London.futures....sterling.tonne.)
# Rename GBP.tonne.futures variables
colnames(GBP.tonne.futures) = c('Date', 'Price')

# Turn date var into type date
GBP.tonne.futures$Date = as.Date(GBP.tonne.futures$Date, format = c("%d/%m/%Y"))

# Turn Price var into type double 
GBP.tonne.futures$Price = as.numeric(gsub(",", "", GBP.tonne.futures$Price))

# Order Data by Date
GBP.tonne.futures = GBP.tonne.futures[order(GBP.tonne.futures$Date),]

# Check variable format
str(GBP.tonne.futures)

# Check for missing data
which(is.na(GBP.tonne.futures))
...
inds <- seq(head(GBP.tonne.futures$Date, n = 1), 
            tail(GBP.tonne.futures$Date, n = 1), 
            by = "day")

substr(head(GBP.tonne.futures$Date, n = 1), 1, 4)

# USE # DAYS IN DATSET FOR FREQ
futures.ts = ts(GBP.tonne.futures$Price,     
                start = c(substr(head(GBP.tonne.futures$Date, n = 1), 1, 4), as.numeric(format(inds[1], "%j"))),
                frequency = 365)
...
#..............................................................................#
####   3. CONVERT TO SAME TIME SCALE                                        ####
#..............................................................................#

# Want monthly data points for all variables (Deforestation & Future Prices)
# will take the monthly average
inds <- seq(head(GBP.tonne.futures$Date, n = 1), 
            tail(GBP.tonne.futures$Date, n = 1), 
            by = "day")

futures.ts = ts(apply.monthly(as.xts(GBP.tonne.futures$Price, GBP.tonne.futures$Date), mean),
                start = c(substr(head(GBP.tonne.futures$Date, n = 1), 1, 4), as.numeric(format(inds[1], "%j"))),
                frequency = 12)
...
# Will only consider data from 2010 until the end of 2019
# Future Prices
temp.ts = as.xts(futures.ts)
temp.ts = temp.ts["2010-01-01/2019-12-31"]
futures.ts = ts(as.vector(temp.ts),
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#### 6.b Residual Plots, Correlograms, & Q-test ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Plot model.010
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#### 6.c Residual^2 Plots, Correlograms, & Q-test ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
# Std.residuals^2 are WN
# alpha2 & beta1(pval=0.678) significant
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#### 7.b Function for Out-of-Sample Validation ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#### 7.c  Out-of-Sample Measurements data frames ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#### 7.e  Plot of selected model vs Data ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
plot(model.010.garch.21)
# numbers 3 (cond. SD) & 11 (acf std. residuals)

testModel=garchFit( ~ garch(2,1), data = d.log.futures.ts, include.mean = F, cond.dist = "QMLE")
summary(testModel)

Testpred = predict(model.010.garch.21, n.ahead = 50)
ts.plot(log.futures.ts, Testpred$pred, lty = c(1,3), col=c(5,2))
Testpred$pred
